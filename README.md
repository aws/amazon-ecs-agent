...
| `ECS_ENGINE_TASK_RESOURCE_WAIT_TIMEOUT` | 10m | Timeout before giving up on acquiring task resources. Tasks that do not have enough resources to run are placed in a queue. Once a task reaches this timeout, it is removed from the queue and set to terminal status. The accounted resources are CPU, memory, static host ports, and GPU. Minimum value is 1 minute. If user sets a value below the minimum it will be set to the default value. | 5m | 5m |
