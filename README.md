...
| `ECS_INSTANCE_ATTRIBUTES` | `{"stack": "prod"}` | A map of custom attributes, in JSON form, to apply to your container instances. These attributes take effect only during **initial registration**. Once the agent has joined an ECS cluster, use the PutAttributes API to add additional attributes. For further information, please have a look at [additional documentation] (http://docs.aws.amazon.com/AmazonECS/latest/developerguide/ecs-agent-config.html).| `{}` | `{}` |
