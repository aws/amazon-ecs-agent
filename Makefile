...
GOFMTFILES:=$(shell find ./agent ./ecs-agent -not -path './agent/vendor/*' -not -path './ecs-agent/vendor/*' -not -path './ecs-agent/daemonimages/csidriver/vendor/*' -not -path './ecs-agent/gpu/dcgmclient/vendor/*' -type f -iregex '.*\.go')
...
gomod:
	...
	cd ./ecs-agent/gpu/dcgmclient && go mod tidy && go mod vendor
