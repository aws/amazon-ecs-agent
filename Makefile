...
gogenerate:
	PATH=$(PATH):$(shell pwd)/scripts go generate -x ./agent/... ./ecs-agent/... ./ecs-init/...
	$(MAKE) goimports

gogenerate-agent:
	PATH=$(PATH):$(shell pwd)/scripts go generate -x ./agent/...
	$(MAKE) goimports

gogenerate-ecs-agent:
	PATH=$(PATH):$(shell pwd)/scripts go generate -x ./ecs-agent/...
	$(MAKE) goimports
