...
func setup(t *testing.T,
	...
	client, err := NewECSClient(credentials.AnonymousCredentials, mockCfgAccessor, ec2MetadataClient, agentVer, osType,
		options...)
...
func applyMockCfgAccessorDefaults(cfgAccessor *mock_config.MockAgentConfigAccessor) {
	...
	cfgAccessor.EXPECT().OSType().Return("linux").AnyTimes()
...
func TestFIPSEndpointStateOnFIPSEnabledHosts(t *testing.T) {
	...
	client, err := NewECSClient(credentials.AnonymousCredentials, cfgAccessor, ec2.NewBlackholeEC2MetadataClient(),
		agentVer, osType, WithFIPSDetected(true))
	assert.NoError(t, err)
...
func TestDiscoverPollEndpointCacheTTLSet(t *testing.T) {
	...
	client, err := NewECSClient(credentials.AnonymousCredentials, newMockConfigAccessor(ctrl, nil),
		ec2.NewBlackholeEC2MetadataClient(), agentVer, osType,
		WithDiscoverPollEndpointCacheTTL(&async.TTL{Duration: ttlDuration}))
	assert.NoError(t, err)
...
func TestDiscoverPollEndpointCacheTTLNotSet(t *testing.T) {
	...
	client, err := NewECSClient(credentials.AnonymousCredentials, newMockConfigAccessor(ctrl, nil),
		ec2.NewBlackholeEC2MetadataClient(), agentVer, osType,
		WithDiscoverPollEndpointCacheTTL(nil))
	assert.NoError(t, err)
