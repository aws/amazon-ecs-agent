...
func (api *EBSDiscoveryClient) ConfirmEBSVolumeIsAttached(deviceName, volumeID string) (string, error) {
	...
	if err != nil {
		return nil, errors.Wrapf(err, "failed to run ebsnvme-id.exe: %s", string(output))
	}

	_, err = parseExecutableOutput(output, volumeID, deviceName)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to parse ebsnvme-id.exe output for volumeID: %s and deviceName: %s",
			volumeID, deviceName)
	}
