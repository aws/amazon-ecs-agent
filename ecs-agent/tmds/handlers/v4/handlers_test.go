...
const (
	...
	// Field order matches moby v29's container.StatsResponse struct layout
	// (id/name first), which differs from the old docker v25 types.StatsJSON
	// (embedded Stats first). The serialized data is otherwise identical.
	containerStatsResponseJSON = `{"id":"%s","name":"%s","read":"0001-01-01T00:00:00Z",` +
		...
		`"storage_stats":{},"preread":"0001-01-01T00:00:00Z","precpu_stats":{"cpu_usage":{"total_usage":0,"usage_in_kernelmode":0,` +
		`"usage_in_usermode":0},"throttling_data":{"periods":0,"throttled_periods":0,"throttled_time":0}},` +
		`"network_rate_stats":{"rx_bytes_per_sec":%d,"tx_bytes_per_sec":%d}}`
