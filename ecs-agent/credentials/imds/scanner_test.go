...
func TestScanNamespace(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "fetch failure with an assume-role-failed entry returns no error",
			setupMock: func(m *mockec2.MockEC2MetadataClient) {
				m.EXPECT().GetMetadata("iam-ecs-1/info").Return(
			...
			expectedFailedRoles: []AssumeRoleFailedIAMRole{
				{TaskID: testTaskID2, RoleType: credentials.ExecutionRoleType},
			},
			expectedMetrics: []metricExpectation{
				{
			...
			// A recorded failed role suppresses the namespace-level error even
			// though the delivered credential's fetch failed; the failed fetch
			// still leaves LastUpdated uncached so it is retried next scan.
...
func TestScan(t *testing.T) {
	tests := []struct {
		...
		{
			name: "failed role aggregated across namespaces suppresses scan error",
			setupMock: func(m *mockec2.MockEC2MetadataClient) {
				key1 := testTaskID1 + "-" + credentials.ApplicationRoleType
				m.EXPECT().GetMetadata("").Return("iam-ecs-1\niam-ecs-2", nil)
