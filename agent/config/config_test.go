...
func TestEnvironmentConfig(t *testing.T) {
	...
	assert.True(t, conf.DeleteNonECSImagesEnable, "Wrong value for DeleteNonECSImagesEnable")
...
func TestDeleteNonECSImagesEnable(t *testing.T) {
	defer setTestRegion()()
	defer setTestEnv("ECS_ENABLE_UNTRACKED_IMAGE_CLEANUP", "true")()
	cfg, err := NewConfig(ec2.NewBlackholeEC2MetadataClient())
	assert.NoError(t, err)
	assert.True(t, cfg.DeleteNonECSImagesEnable, "Wrong value for TaskIAMRoleEnabledForNetworkHost")
}
