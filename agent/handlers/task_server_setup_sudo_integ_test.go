...
func skipForUnsupportedTc(t *testing.T) {
	cmdCtx, cancel := context.WithTimeout(context.Background(), requestTimeoutInSeconds)
	defer cancel()

	_, err := exec.CommandContext(cmdCtx, "bash", "-c", "tc -j q show").CombinedOutput()
	if err != nil {
		t.Skipf("Error occurred while running tc utility. Current installed version of the tc utility must have the required flag/configuration available.")
	}
}
