...
func introspectionServerSetup(containerInstanceArn string,
	ecsClient api.ECSClient,
	taskEngine handlersutils.DockerStateResolver,
	cfg *config.Config) *http.Server {
	paths := []string{
		v1.AgentMetadataPath,
		v1.TaskContainerMetadataPath,
		v1.TaskWithTagsContainerMetadataPath,
		v1.LicensePath,
	}
	...
	v1HandlersSetup(serverMux, containerInstanceArn, ecsClient, taskEngine, cfg)
...
func v1HandlersSetup(serverMux *http.ServeMux,
	...
	serverMux.HandleFunc(v1.TaskContainerMetadataPath, v1.TaskContainerMetadataHandler(containerInstanceArn, ecsClient, taskEngine, false))
	serverMux.HandleFunc(v1.TaskWithTagsContainerMetadataPath, v1.TaskContainerMetadataHandler(containerInstanceArn, ecsClient, taskEngine, true))
...
func ServeIntrospectionHTTPEndpoint(containerInstanceArn string, ecsClient api.ECSClient, taskEngine engine.TaskEngine, cfg *config.Config) {
	...
	server := introspectionServerSetup(containerInstanceArn, ecsClient, dockerTaskEngine, cfg)
	for {
		once := sync.Once{}
