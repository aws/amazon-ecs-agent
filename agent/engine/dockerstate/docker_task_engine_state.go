...
func (state *DockerTaskEngineState) AddEBSAttachment(ebsAttachment *apiresource.ResourceAttachment) {
	...
	if _, ok := state.ebsAttachments[volumeId]; !ok {
		...
		// this is what's causing the race condition?
		seelog.Debugf("Successfully added EBS attachment: %v", ebsAttachment.String())
	} else {
		seelog.Debugf("Duplicate ebs attachment information: %v", ebsAttachment.String())
	}
}
...
func (state *DockerTaskEngineState) RemoveEBSAttachment(volumeId string) {
	...
	if ebs, ok := state.ebsAttachments[volumeId]; ok {
		...
		seelog.Debugf("Successfully deleted EBS attachment: %v", ebs.String())
	} else {
