...
func (m *manager) CreateInstanceTask(cfg *config.Config, appnetAgentLoader serviceconnect.Loader) (*apitask.Task, error) {
	imageName, err := appnetAgentLoader.GetLoadedImageName()
	if err != nil {
		return nil, err
	}
	task := &apitask.Task{
