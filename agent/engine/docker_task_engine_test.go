...
// TestEnableMetadata checks if container metadata methods are run if feature is enabled
func TestEnableContainerMetadata(t *testing.T) {
	enableMetadataConfig := defaultConfig
	enableMetadataConfig.ContainerMetadataEnabled = true
	ctrl, client, mockTime, taskEngine, credentialsManager, imageManager, metadataManager := mocks(t, &enableMetadataConfig)
	defer ctrl.Finish()
