...
// This tests the case where a task can consume resources even when there are tasks ahead in the queue that cannot.
func TestTaskWaitForHostResourcesWithDifferentRequests(t *testing.T) {
	...
	// acquire for host resources for task arn0
	taskEngine.managedTasks["arn0"].waitForHostResources()

	// acquire for host resources for task arn1
	...
	// acquire for host resources for task arn2
	taskEngine.managedTasks["arn2"].waitForHostResources()
