...
func TestFirelensFluentbit(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:                   "fluent-bit, root user",
			firelensContainerImage: testFluentBitImage,
			firelensContainerUser:  "0",
		},
		{
			name:                   "fluent-bit, non-root user",
			firelensContainerImage: testFluentBitImage,
			firelensContainerUser:  "1234:foo",
		},
	}
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			firelensTask := createFirelensTask(t, tc.firelensContainerImage, tc.firelensContainerUser)

			// Start the Firelens task
			go taskEngine.AddTask(firelensTask)

			// Validate the Firelens task and container statuses
			verifyTaskStatusBasedOnAction(t, taskEngine, "start")

			// Verify Firelens data directory configuration
			firelensDataDir := filepath.Join(testDataDir, "firelens", firelensTask.Arn)
			verifyFirelensDataDir(t, firelensDataDir, tc.firelensContainerUser)

			// Verify app logs from CloudWatch
			verifyFirelensLogs(t, firelensTask)

			// Stop and cleanup the Firelens task
			stopAndCleanupFirelensTask(t, cfg, firelensTask, taskEngine, firelensDataDir)

			// Validate the Firelens task and container statuses
			verifyTaskStatusBasedOnAction(t, taskEngine, "stop")
		})
	}
}
