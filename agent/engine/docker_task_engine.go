...
func (engine *DockerTaskEngine) AddTask(task *apitask.Task) {
	...
	if task.IsEBSTaskAttachEnabled() {
		if ebsCsiDaemonManager, ok := engine.daemonManagers["ebs-csi-driver"]; ok {
			if engine.ebsCsiDriver == nil {
				engine.ebsCsiDriver, err = ebsCsiDaemonManager.CreateDaemonTask()
				if err != nil {
				...
				engine.AddTask(engine.ebsCsiDriver)
				logger.Info("docker_task_engine: Added EBS CSI task to engine")
			}
		} else {
			logger.Error("Unable to find ebs-csi-driver in engine")
		}
	}
