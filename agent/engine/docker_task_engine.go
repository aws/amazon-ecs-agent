...
func (engine *DockerTaskEngine) stopContainer(task *apitask.Task, container *apicontainer.Container) dockerapi.DockerContainerMetadata {
	...
	return engine.stopDockerContainer(dockerID, apiTimeoutStopContainer)
}
...
func (engine *DockerTaskEngine) stopDockerContainer(dockerID string, apiTimeoutStopContainer time.Duration) dockerapi.DockerContainerMetadata {
	...
	if md.Error != nil && md.Error.ErrorName() == dockerapi.DockerTimeoutErrorName {
		seelog.Warnf("Unable to stop container after %d attempts timed out; giving up and marking container as stopped", stopContainerMaxRetryCount)
		md = dockerapi.DockerContainerMetadata{
			DockerID:   dockerID,
			FinishedAt: time.Now(),
		}
	}
