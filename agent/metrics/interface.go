...
type MetricsEngine_ interface {
	...
	// for incoming requests to expose metrics. The running Prometheus
	// container alongside Agent will 'scrape' Agent at pre-configurated
	// intervals. When a request is made, this method ensures that the
	// collected metrics are published
	publishMetrics()
}
