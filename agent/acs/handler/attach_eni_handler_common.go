...
func (deHandler *dockerENIHandler) HandleENIAttachment(ea *apieni.ENIAttachment) error {
	...
	if eniAttachment, ok := deHandler.state.ENIByMac(mac); ok {
		...
		return eniAttachment.StartTimer(deHandler.handleENIAckTimeout)
	}
	if err := deHandler.addENIAttachmentToState(ea); err != nil {
...
func (deHandler *dockerENIHandler) addENIAttachmentToState(ea *apieni.ENIAttachment) error {
	attachmentType := ea.AttachmentType
	attachmentARN := ea.AttachmentARN
	taskARN := ea.TaskARN
	mac := ea.MACAddress
	...
	if err := ea.StartTimer(deHandler.handleENIAckTimeout); err != nil {
		return err
	}
	...
	deHandler.state.AddENIAttachment(ea)
	if err := deHandler.dataClient.SaveENIAttachment(ea); err != nil {
...
func (deHandler *dockerENIHandler) handleENIAckTimeout() {
	eniAttachment, ok := deHandler.state.ENIByMac(deHandler.mac)
	if !ok {
		seelog.Warnf("Ignoring unmanaged ENI attachment mac=%s", deHandler.mac)
		return
	}
	if !eniAttachment.IsSent() {
		seelog.Warnf("Timed out waiting for ENI ack; removing ENI attachment record %s", eniAttachment.String())
		deHandler.removeENIAttachmentData(deHandler.mac)
		deHandler.state.RemoveENIAttachment(deHandler.mac)
	}
}

func (deHandler *dockerENIHandler) removeENIAttachmentData(mac string) {
	attachmentToRemove, ok := deHandler.state.ENIByMac(mac)
	if !ok {
	...
	if err != nil {
		...
		err = deHandler.dataClient.DeleteENIAttachment(attachmentId)
		if err != nil {
