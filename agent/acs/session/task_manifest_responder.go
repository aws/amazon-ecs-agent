...
func (tc *taskComparer) CompareRunningTasksOnInstanceWithManifest(
	message *ecsacs.TaskManifestMessage) ([]types.TaskIdentifier, error) {
	tasksOnInstance, err := tc.taskEngine.ListTasks()
	if err != nil {
		return nil, err
	}

	tasksToBeKilled := make([]types.TaskIdentifier, 0)
	for _, task := range tasksOnInstance {
		...
		if task.GetDesiredStatus() == apitaskstatus.TaskRunning {
			...
			if !taskPresent {
				tasksToBeKilled = append(tasksToBeKilled, types.TaskIdentifier{
