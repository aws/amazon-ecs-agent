...
func (ts *taskStopper) StopTask(taskARN string) {
	...
	if isPresent {
		...
		// Create a minimal copy of the task with its desired status set to STOPPED.
		taskWithDesiredStatusStopped := taskWithARNAndDesiredStatus(task.Arn, apitaskstatus.TaskStopped)
		// Signal to task engine that the task should be stopped.
...
// taskWithARNAndDesiredStatus creates a task with ARN `taskARN` and desired status `desiredStatus`.
func taskWithARNAndDesiredStatus(taskARN string, desiredStatus apitaskstatus.TaskStatus) *apitask.Task {
