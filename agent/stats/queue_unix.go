...
func aggregateOSDependentStats(dockerStat, lastStatBeforeLastRestart *container.StatsResponse) *container.StatsResponse {
	// CPU stats.
...
func aggregateBlockStat(dockerStatBlockStatSlice *[]container.BlkioStatEntry,
	lastStatBeforeLastRestartStatBlockStatSlice []container.BlkioStatEntry) {
	if len(*dockerStatBlockStatSlice) == 0 && len(lastStatBeforeLastRestartStatBlockStatSlice) == 0 {
		return
	}

	var aggregatedBlockStatSlice []container.BlkioStatEntry
...
func addBlockStatEntriesFromSliceToMap(statSlice []container.BlkioStatEntry, statMap map[BlockStatKey]BlockStatValue) {
...
func addCorrespondingMapEntriesOfSliceToAggregatedSlice(statMap map[BlockStatKey]BlockStatValue,
	statSlice []container.BlkioStatEntry, aggregatedSlice *[]container.BlkioStatEntry) {
	for _, blockStat := range statSlice {
		blkStatKey := BlockStatKey{blockStat.Major, blockStat.Minor, blockStat.Op}
		blkStatVal, ok := statMap[blkStatKey]
		if ok && !blkStatVal.HasBeenRetrieved {
			*aggregatedSlice = append(*aggregatedSlice, container.BlkioStatEntry{
