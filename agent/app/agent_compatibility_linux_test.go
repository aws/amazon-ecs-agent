...
func TestCompatibilityEnabledSuccess(t *testing.T) {
	ctrl, creds, _, images, _, _, stateManagerFactory, saveableOptionFactory, execCmdMgr, appnetAgentLoader := setup(t)
	defer ctrl.Finish()
	...
	_, _, err := agent.newTaskEngine(containerChangeEventStream, creds, dockerstate.NewTaskEngineState(), images, execCmdMgr, appnetAgentLoader)
