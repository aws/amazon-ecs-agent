...
type ecsAgent struct {
	...
	appNetLoader                serviceconnect.Loader
...
func (agent *ecsAgent) doStart(containerChangeEventStream *eventstream.EventStream,
	...
	taskEngine, currentEC2InstanceID, err := agent.newTaskEngine(
		containerChangeEventStream, credentialsManager, state, imageManager, execCmdMgr, agent.appNetLoader)
	if err != nil {
...
func (agent *ecsAgent) newTaskEngine(containerChangeEventStream *eventstream.EventStream,
	...
	appnetAgentloader serviceconnect.Loader) (engine.TaskEngine, string, error) {
	...
	if !agent.cfg.Checkpoint.Enabled() {
		...
		return engine.NewTaskEngine(agent.cfg, agent.dockerClient, credentialsManager,
			...
			agent.metadataManager, agent.resourceFields, execCmdMgr, appnetAgentloader), "", nil
	}

	savedData, err := agent.loadData(containerChangeEventStream, credentialsManager, state, imageManager, execCmdMgr, appnetAgentloader)
	if err != nil {
	...
	if savedData.ec2InstanceID != "" && savedData.ec2InstanceID != currentEC2InstanceID {
		...
		return engine.NewTaskEngine(agent.cfg, agent.dockerClient, credentialsManager,
			...
			agent.resourceFields, execCmdMgr, appnetAgentloader), currentEC2InstanceID, nil
	}
