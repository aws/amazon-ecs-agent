...
func TestNewTaskEngineRestoreFromCheckpointNoEC2InstanceIDToLoadHappyPath(t *testing.T) {
	ctrl, credentialsManager, _, imageManager, _,
		dockerClient, stateManagerFactory, saveableOptionFactory, execCmdMgr, appnetAgentLoader := setup(t)
	defer ctrl.Finish()
	...
	_, instanceID, err := agent.newTaskEngine(eventstream.NewEventStream("events", ctx),
		credentialsManager, dockerstate.NewTaskEngineState(), imageManager, execCmdMgr, appnetAgentLoader)
...
func TestNewTaskEngineRestoreFromCheckpoint(t *testing.T) {
	...
	_, instanceID, err := agent.newTaskEngine(eventstream.NewEventStream("events", ctx),
		credentialsManager, state, imageManager, execCmdMgr, appnetAgentLoader)
	assert.NoError(t, err)
