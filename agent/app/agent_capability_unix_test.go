...
func TestNvidiaDriverCapabilitiesUnix(t *testing.T) {
	...
	expectedCapabilityNames := []string{
		...
		attributePrefix + capabilityNvidiaDriverVersionInfix + nvidiaDriverVersion, // keep old name only for backward compatibility
	}
	...
	// Test key/value nvidia driver version capability
	expectedCapabilities = append(expectedCapabilities,
		&ecs.Attribute{Name: aws.String(attributePrefix + capabilityNvidiaDriverVersion), Value: aws.String(nvidiaDriverVersion)})
