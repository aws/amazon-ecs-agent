...
func returnExpectedBindMounts() ([]string, []string) {
	expectedFluentdBindMounts := []string{
		`testDataDirOnHost\firelens\task-id\config\:C:\data\fluentd\etc\`,
	}
	expectedFluentBitBindMounts := []string{
		`testDataDirOnHost\firelens\task-id\config\:C:\data\fluent-bit\etc\`,
	}
	return expectedFluentdBindMounts, expectedFluentBitBindMounts
}
