...
func TaskFromACS(acsTask *ecsacs.Task, envelope *ecsacs.PayloadMessage) (*Task, error) {
	...
	// validate the GPU sharing strategy on resource requirements before the task
	// is acted on
	if err := validateResourceRequirements(acsTask); err != nil {
		return nil, err
	}
...
// validateResourceRequirements enforces the invariants of the ACS
// ResourceRequirement.sharingStrategy union. sharingStrategy is modeled as a
// union (exactly one known member), but the agent's ACS model is generated by
// the aws-sdk-go v1 generator, which has no union type, so it is emitted as a
// plain struct with optional members. An empty strategy, an unknown future
// strategy (its member dropped during unmarshal), or a missing required field
// would all deserialize silently and be misread as a whole-GPU request. This
// re-imposes the union guarantee at the point wire data enters the agent:
// exactly one known strategy must be set, with its required fields present, or
// the whole task payload is rejected.
func validateResourceRequirements(acsTask *ecsacs.Task) error {
	for _, container := range acsTask.Containers {
		if container == nil {
			continue
		}
		for _, rr := range container.ResourceRequirements {
			...
			if rr.SharingStrategy.NvidiaMps == nil {
				return fmt.Errorf("container %q: GPU sharingStrategy is set with no known strategy",
					aws.ToString(container.Name))
			}
			if rr.SharingStrategy.NvidiaMps.Memory == nil {
				return fmt.Errorf("container %q: nvidiaMps.memory is required for the MPS GPU sharing strategy",
					aws.ToString(container.Name))
			}
		}
	}
