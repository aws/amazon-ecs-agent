...
func TestPostUnmarshalWithFirelensContainer(t *testing.T) {
	...
	assert.Equal(t, testDataDir+taskIDSuffix, firelensResource.GetResourceDir())
...
func TestAddFirelensContainerBindMounts(t *testing.T) {
	expectedFluentdBindMounts, expectedFluentBitBindMounts := returnExpectedBindMounts()
	cfg := &config.Config{
	...
	testCases := []struct {
	...
	}{
		{
			...
			cfg:                cfg,
			shouldFail:         false,
			expectedBindMounts: expectedFluentdBindMounts,
		},
		{
			...
			cfg:                cfg,
			shouldFail:         false,
			expectedBindMounts: expectedFluentBitBindMounts,
		},
		{
