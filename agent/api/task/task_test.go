...
func TestTaskFromACSServiceNameMissing(t *testing.T) {
	taskFromACS := types.Task{} // No service name
	seqNum := int32(42)
	task, err := TaskFromACS(&taskFromACS, &ecsacs.PayloadInput{SeqNum: &seqNum})
	assert.Nil(t, err, "Should be able to handle acs task")
...
func containerFromACS(name string, containerPort int32, hostPort int32, networkMode string) types.Container {
	...
	if portMapping != nil {
		container.PortMappings = []types.PortMapping{
			*portMapping,
		}
	}
	return container
