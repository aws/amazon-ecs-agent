...
func TestDockerHostConfigPortBinding(t *testing.T) {
		...
		Containers: []*apicontainer.Container{
			{
				...
				Ports: []apicontainer.PortBinding{
					{
						ContainerPortRange: aws.String("999-1009"),
						BindIP:             "",
						Protocol:           apicontainer.TransportProtocolTCP,
					},
					{
	...
	testCases := []struct {
		testName                      string
		testTask                      *Task
		getHostPortRange              func(containerPortRange string, protocol string) (string, error)
		...
		},
		{
			...
			testTask: testTask2,
			getHostPortRange: func(containerPortRange string, protocol string) (string, error) {
				if containerPortRange == "1-3" {
					return "", errors.New("couldn't find host ports")
				}
				return "99-109", nil
			},
			expectedPortBinding: nat.PortMap{
				nat.Port("999-1009/tcp"): []nat.PortBinding{{HostPort: "99-109"}},
				nat.Port("1-3/udp"):      []nat.PortBinding{{HostPort: "0"}},
			},
			expectedContainerPortSet: map[int]struct{}{
				1: {},
				2: {},
	...
	for _, tc := range testCases {
		t.Run(tc.testName, func(t *testing.T) {
			...
			if !reflect.DeepEqual(config.PortBindings, tc.expectedPortBinding) {
				t.Error("Expected port bindings to be resolved, was: ", config.VolumesFrom)
			}
