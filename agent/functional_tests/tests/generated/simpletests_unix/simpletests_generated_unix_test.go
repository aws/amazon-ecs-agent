...
// TestTaskSharedVolume Verify that shared Docker volume works as expected
func TestTaskSharedVolume(t *testing.T) {
	...
	td, err := GetTaskDefinition("task-shared-vol")
	if err != nil {
		t.Fatalf("Could not register task definition: %v", err)
	}
	testTasks, err := agent.StartMultipleTasks(t, td, 1)
