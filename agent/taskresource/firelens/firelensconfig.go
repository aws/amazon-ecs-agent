...
func (firelens *FirelensResource) generateConfig() (generator.FluentConfig, error) {
	...
	firelens.addSocketInput(config)
	firelens.addTCPInput(config)
	...
	if firelens.externalConfigType == ExternalConfigTypeFile {
		...
		config.AddExternalConfig(firelens.getS3ConfPath(), generator.AfterFilters)
	}
...
func (firelens *FirelensResource) addTCPInput(config generator.FluentConfig) {
	if firelens.networkMode == bridgeNetworkMode || firelens.networkMode == awsvpcNetworkMode || firelens.networkMode == "" {
		var inputMap map[string]string
		var inputName, inputBindValue string
		if firelens.networkMode == bridgeNetworkMode || firelens.networkMode == "" {
