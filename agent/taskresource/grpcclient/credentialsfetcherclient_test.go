...
func (*mockCredentialsFetcherServer) AddKerberosLease(ctx context.Context, req *pb.CreateKerberosLeaseRequest) (*pb.CreateKerberosLeaseResponse, error) {
	...
	return &pb.CreateKerberosLeaseResponse{LeaseId: leaseid, CreatedKerberosFilePaths: []string{"/var/credentials-fetcher/krbdir/123456/ccname_webapp01", "/var/credentials-fetcher/krbdir/123456/ccname_webapp02"}}, nil
}
...
func (*mockCredentialsFetcherServer) DeleteKerberosLease(ctx context.Context, req *pb.DeleteKerberosLeaseRequest) (*pb.DeleteKerberosLeaseResponse, error) {
	...
	return &pb.DeleteKerberosLeaseResponse{LeaseId: leaseid, DeletedKerberosFilePaths: []string{"/var/credentials-fetcher/krbdir/123456/ccname_webapp01", "/var/credentials-fetcher/krbdir/123456/ccname_webapp02"}}, nil
}
...
func TestCredentialsFetcherClient_AddKerberosLease(t *testing.T) {
	/*tests := []struct {
		...
		},
		{
			...
			CredentialsFetcherResponse{LeaseId: leaseid, KerberosTicketPaths: []string{"/var/credentials-fetcher/krbdir/123456/ccname_webapp01", "/var/credentials-fetcher/krbdir/123456/ccname_webapp02"}},
		},
	}*/

	//ctx := context.Background()
	os.Setenv("CREDENTIALS_FETCHER_HOST_DIR", "/var/credentials-fetcher/socket/credentials_fetcher.sock")
	defer os.Unsetenv("CREDENTIALS_FETCHER_HOST_DIR")
	conn, _ := GetGrpcClientConnection()
	response, _ := NewCredentialsFetcherClient(conn, time.Minute).AddKerberosLease(context.Background(), []string{credspec_webapp01})
	...
	//			t.Error("error: expected", tt.response, "received", response)
	//		}
	//	})
	//}
}
...
func TestCredentialsFetcherClient_DeleteKerberosLease(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			response, _ := NewCredentialsFetcherClient(conn, time.Minute).DeleteKerberosLease(context.Background(), tt.leaseid)
