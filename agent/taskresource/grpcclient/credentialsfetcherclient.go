...
func GetGrpcClientConnection() (*grpc.ClientConn, error) {
	...
	if err != nil {
		seelog.Debugf("could not find path to credentials fetcher host dir : %v", err)
		return nil, err
	}

	conn, err := grpc.Dial(address, grpc.WithTransportCredentials(insecure.NewCredentials()))
	if err != nil {
		seelog.Debugf("could not initialize client connection %v", err)
		return nil, err
	}
	return conn, nil
...
func (c CredentialsFetcherClient) AddKerberosLease(ctx context.Context, credentialspecs []string) (CredentialsFetcherResponse, error) {
	if len(credentialspecs) == 0 {
		seelog.Infof("credentialspecs request should not be empty")
	...
	if err != nil {
		...
		seelog.Debugf("could not create kerberos tickets: %v", err)
		return CredentialsFetcherResponse{}, err
	}
	seelog.Infof("deleted kerberos associated with lease_id: %s", response.GetLeaseId())
...
func (c CredentialsFetcherClient) DeleteKerberosLease(ctx context.Context, leaseid string) (CredentialsFetcherResponse, error) {
	if len(leaseid) == 0 {
		seelog.Infof("invalid leaseid provided")
		return CredentialsFetcherResponse{}, nil
	}
	...
	if err != nil {
		...
		seelog.Debugf("could not delete kerberos tickets: %v", err)
		return CredentialsFetcherResponse{}, err
	}
	seelog.Infof("deleted kerberos associated with lease_id: %s", response.GetLeaseId())
