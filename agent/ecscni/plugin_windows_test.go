	...
	gomock.InOrder(
		...
		libcniClient.EXPECT().AddNetwork(gomock.Any(), gomock.Any(), gomock.Any()).Return(&current.Result{}, nil).Do(
			func(ctx context.Context, net *libcni.NetworkConfig, rt *libcni.RuntimeConf) {
				assert.Equal(t, pluginName, net.Network.Type, "first plugin should be vpc-shared-eni")
			}),
	)
...
func TestConstructNetworkConfig(t *testing.T) {
	...
	assert.Equal(t, ipv4Addr, taskBridgeConfig.ENIIPAddress)
	assert.Equal(t, ipv4Addr, taskBridgeConfig.IPAddress)
	assert.Equal(t, taskENI.MacAddress, taskBridgeConfig.ENIMACAddress)
	assert.Equal(t, subnet[0], taskBridgeConfig.GatewayIPAddress)
}
