...
func (e *Engine) PreStart() error {
	...
	// Add the EBS CSI log directory
	err = os.MkdirAll(config.EBSCSILogDirectory(), mountFilePermission)
	if err != nil {
		return engineError("could not create daemon log directory", err)
	}
